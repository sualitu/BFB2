...
namespace Assets.BattleForBetelgeuse.Animations.Environment.Discs {
    [ExecuteInEditMode]
            ...
            SgtProceduralMesh.Clear();
            {
                if (SegmentDetail >= 3)
                {
                    ...
                    for (var i = 0; i <= SegmentDetail; i++)
                    {
                        ...
                        SgtProceduralMesh.PushCoord1(Min, coord * InnerRadius);
                        SgtProceduralMesh.PushCoord1(Max, coord * OuterRadius);
