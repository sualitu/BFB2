...
namespace BattleForBetelgeuse {
  ...
  public class RTSCamera : MonoBehaviour {
    ...
    void Update() {
      ...
      translation += (new Vector3(Input.GetAxis("Horizontal") * Settings.Camera.KeyboardScrollSpeed, 0, 
                                  Input.GetAxis("Vertical")   * Settings.Camera.KeyboardScrollSpeed));

      if(Input.GetMouseButton(2)) { 
        // Hold button and drag camera around
        translation -= new Vector3(Input.GetAxis("Mouse X") * Settings.Camera.DragSpeed * Time.deltaTime, 0,
