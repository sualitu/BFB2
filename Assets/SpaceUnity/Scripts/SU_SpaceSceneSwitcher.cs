...
public class SU_SpaceSceneSwitcher : MonoBehaviour {
	...
	void Start () {
		...
		if (staticMode == Mode.LOAD_ALL_AT_STARTUP) {
			...
			foreach (GameObject _spaceScene in spaceScenes) {
                ...
                GameObject _instantiated = (GameObject) Instantiate(_spaceScene, _spaceScene.transform.position, _spaceScene.transform.localRotation); // , new Vector3 (0,0,0), new Quaternion(0, 0, 0, 0)
    ...
    static public void SwitchToRandom()
    {
        if (staticSpaceScenes.Count > 0)
        {
            Switch(staticSpaceScenes[Random.Range(0, staticSpaceScenes.Count)].name);
        }
    }
